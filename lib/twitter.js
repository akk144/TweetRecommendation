	...
	client.get('followers/ids', list_criteria,function(error,followers,response) {
		...
		async.series([
			function getLastPost(callback) {
				getLastPostOfAllFollowers(followers.ids,time,days,function() {
					return callback();
				});
			},
...
function getLastPostOfAllFollowers(ids,time,days,done) {
	async.forEach(ids,function(user,cb)  {
		var crit = {user_id : user,count : 1};
		client.get('statuses/user_timeline',crit,function(err,tweet,resp) {
			if(!_.isEmpty(tweet) &&  !_.isEmpty(tweet[0])) {
